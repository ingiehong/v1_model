import math

import numpy as np
import h5py
import matplotlib.pyplot as plt
import pandas as pd
import matplotlib.pylab as pylab
import os
import json
from scipy.optimize import curve_fit  # Yidong: Added this import for curve fitting

""" Calculate the above-baseline firing rates and corresponding OSI&DSI of neurons in the
double-gray stimulation scheme. 
"""

def calculateFiringRate(gids, ts, numNrns, gray_screen=0.0, length=12000.0):
    print("Calculating Firing Rate")
    gids = gids[np.where(ts > gray_screen)[0]]
    ts = ts[np.where(ts > gray_screen)[0]]
    gids = gids[np.where(ts <= (length - gray_screen))[0]]
    gid_bins = np.arange(0 - 0.5, numNrns + 0.5, 1)

    hist, bins = np.histogram(gids, bins=gid_bins)

    if gray_screen > 0:
        mean_firing_rates = hist / ((length - 2 * gray_screen) / 1000.)
    else:
        mean_firing_rates = hist / (length / 1000.)
    return mean_firing_rates


# Yidong: Calculate the firing rate within a time interval defined by t0 (ms) and t1 (ms)
def calculateFiringRateWithinInterval(gids, ts, numNrns, t0, t1):
    gids = gids[np.where(ts > t0)[0]]
    ts = ts[np.where(ts > t0)[0]]
    gids = gids[np.where(ts <= t1)[0]]
    gid_bins = np.arange(0 - 0.5, numNrns + 0.5, 1)

    hist, bins = np.histogram(gids, bins=gid_bins)

    mean_firing_rates = hist / ((t1 - t0) / 1000.)
    return mean_firing_rates


# Yidong: The above-baseline firing rates equal to the firing rates in the stimulation period
# minus the firing rates in the previous blank period.
def calculateAboveBaselineFiringRate(gids, ts, numNrns, gray_screen=4000.0, length=12000.0):
    mean_firing_rates_before_stimuli = calculateFiringRateWithinInterval(gids, ts, numNrns, 0.0, gray_screen)
    mean_firing_rates_during_stimuli = calculateFiringRateWithinInterval(gids, ts, numNrns, gray_screen,
                                                                         length - gray_screen)
    return mean_firing_rates_during_stimuli - mean_firing_rates_before_stimuli


def calculate_Rates_DF(numNrns, trials=10, oris=np.arange(0, 360, 30)):  # Changed orientations to 30-degree increments

    Rates_DF = pd.DataFrame(index=range(numNrns * len(oris)),
                            columns=['DG_angle', 'node_id', 'Avg_rate(Hz)', 'SD_rate(Hz)'])

    for i, ori in enumerate(oris):
        firingRatesTrials = np.zeros((trials, numNrns))

        for trial in range(trials):
            spikes_file_name = '12s_SF0.04_TF2.0_ori' + str(float(ori)) + '_c100.0_gs0.5_spikes_trial_' + str(
                trial) + '.txt'

            spikes = np.loadtxt(spikes_file_name, unpack=True)
            ts, gids = spikes
            gids = gids.astype(int)

            firingRates = calculateAboveBaselineFiringRate(gids, ts, numNrns, gray_screen=4000., length=12000.)
            print(spikes_file_name)

            firingRatesTrials[trial, :] = firingRates

        Rates_DF.loc[i * numNrns:(i + 1) * numNrns - 1, 'DG_angle'] = ori
        Rates_DF.loc[i * numNrns:(i + 1) * numNrns - 1, 'node_id'] = np.arange(numNrns)
        Rates_DF.loc[i * numNrns:(i + 1) * numNrns - 1, 'Avg_rate(Hz)'] = np.mean(firingRatesTrials, axis=0)
        Rates_DF.loc[i * numNrns:(i + 1) * numNrns - 1, 'SD_rate(Hz)'] = np.std(firingRatesTrials, axis=0)

    Rates_DF.to_csv('Rates_AboveBaseline_DF.csv', sep=' ', index=False)


def calculate_OSI_DSI_from_DF(Rates_DF):
    numNrns = Rates_DF['node_id'].max() + 1
    OSI_DSI_DF = pd.DataFrame(index=range(numNrns),
                              columns=['node_id', 'DSI', 'OSI', 'preferred_angle', 'max_mean_rate(Hz)'])

    for i in range(numNrns):
        rates = np.array(Rates_DF.loc[Rates_DF['node_id'] == i, 'Avg_rate(Hz)'])
        # Yidong: correct negative firing rates
        rates_corrected = rates - np.min(np.append(rates, 0))
        angles = np.array(Rates_DF.loc[Rates_DF['node_id'] == i, 'DG_angle'])

        if i % 2000 == 0:
            print('cell: ', i)

        if np.max(rates) > 0.0:
            pref_ang = angles[np.argmax(rates)]

            # Calculate DSI
            numerator = [rates_corrected[k] * np.exp(1j * np.deg2rad(theta)) for k, theta in enumerate(angles)]
            denominator = rates_corrected
            DSI = np.abs(np.sum(numerator)) / np.sum(denominator)

            # Calculate OSI
            numerator = [rates_corrected[k] * np.exp(2 * 1j * np.deg2rad(theta)) for k, theta in enumerate(angles)]
            denominator = rates_corrected
            OSI = np.abs(np.sum(numerator)) / np.sum(denominator)
        else:
            DSI = np.nan
            OSI = np.nan
            pref_ang = np.nan

        OSI_DSI_DF.loc[i, 'node_id'] = i
        OSI_DSI_DF.loc[i, 'DSI'] = DSI
        OSI_DSI_DF.loc[i, 'OSI'] = OSI
        OSI_DSI_DF.loc[i, 'preferred_angle'] = pref_ang
        OSI_DSI_DF.loc[i, 'max_mean_rate(Hz)'] = np.max(rates)
        OSI_DSI_DF.loc[i, 'Avg_Rate(Hz)'] = np.mean(rates)

    OSI_DSI_DF.to_csv('OSI_DSI_AboveBaseline_DF.csv', sep=' ', index=False)


if __name__ == "__main__":
    trials = 10
    oris = np.arange(0, 360, 30)  # Changed orientations to 30-degree increments

    nodes_file_name = '../../../v1_biophysical/network/v1_nodes.csv'
    nodes_DF = pd.read_csv(nodes_file_name, sep=' ')
    numNrns = len(nodes_DF)

    calculate_Rates_DF(numNrns, trials=trials, oris=oris)
    print("Done_Rates_DF!")

    Rates_DF = pd.read_csv('Rates_AboveBaseline_DF.csv', sep=' ', index_col=False)
    calculate_OSI_DSI_from_DF(Rates_DF)
    print("Done with all!")
