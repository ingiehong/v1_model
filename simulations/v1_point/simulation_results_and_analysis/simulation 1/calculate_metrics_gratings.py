...
def calculate_Rates_DF(numNrns, trials = 10,  oris = np.arange(0, 360, 45)):
    ...
    for i, ori in enumerate(oris):
        ...
        for trial in range(trials):
            ...
            spikes_file_name = 'full3_GScorrected_PScorrected_3.0sec_SF0.04_TF2.0_ori' + str(float(ori)) + '_c100.0_gs0.5_spikes_trial_' + str(trial) + '.txt'
    ...
    Rates_DF.to_csv('Rates_DF.csv', sep = ' ', index = False)
...
def calculate_OSI_DSI_from_DF(Rates_DF):
    ...
    OSI_DSI_DF.to_csv('OSI_DSI_DF.csv', sep = ' ', index = False)
