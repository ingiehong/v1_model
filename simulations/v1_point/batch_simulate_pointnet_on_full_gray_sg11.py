...
config_template = {
    "manifest": {
        "$BASE_DIR": base_dir,
        "$NETWORK_DIR": "$BASE_DIR/network_dynsyns",
        ...
        "nodes": [
            {
                ...
                "node_types_file": "$NETWORK_DIR/v1_node_types_sg2.csv"
            },
            {
        ...
        "edges": [
            {
                ...
                "edge_types_file": "$NETWORK_DIR/v1_v1_edge_types_sg5.csv"
            },
            {
...
for trial in range(trials):
    ...
    output_dir = os.path.join(base_dir, 'output', f'sg11_12s_full_gray_trial_{trial}')
    os.makedirs(output_dir, exist_ok=True)

    # Update the configuration
    config = config_template.copy()
    lgn_input_file = f"$LGN_DIR/results/12s_full_gray/spikes.trial_{trial}.h5"
    bkg_input_file = f"$BKG_DIR/results/bkg_spikes_n1_fr1000_dt0.25_12s/spikes.trial_{trial}.csv"
    output_spikes_file = f"12s_full_gray_spikes_trial_{trial}.h5"
    output_spikes_csv_file = f"12s_full_gray_spikes_trial_{trial}.csv"
    ...
    if os.path.exists(os.path.join(output_dir, output_spikes_csv_file)):
        ...
        config_filename = os.path.join(output_dir, f"config_full_gray_trial{trial}.json")
        with open(config_filename, 'w') as f:
            json.dump(config, f, indent=2)

        # Run the simulation
        print(f"Running simulation for trial={trial}")
        ...
        if os.path.exists(log_file):
            with open(log_file, 'r') as log:
                ...
                print(f"Log file content for trial={trial}:\n{log_content}")
        else:
